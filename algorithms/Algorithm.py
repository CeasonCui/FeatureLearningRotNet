...
class Algorithm():
    ...
    def set_experiment_dir(self,directory_path):#设置路径
    ...
    def set_log_file_handler(self):#管理日志
    ...
    def init_all_networks(self):#网络初始化
        networks_defs = self.opt['networks']
        self.networks = {}
        self.optim_params = {}

        for key, val in networks_defs.items():#为啥key是networks的key，和下面的networks每个Key更新额东西一样这事逻辑不通
            self.logger.info('Set network %s' % key) #日志
            def_file = val['def_file']#从config里传过来
            net_opt = val['opt']
            self.optim_params[key] = val['optim_params'] if ('optim_params' in val) else None #netwrok的classifier里的一个参数optim_params，这里是各种参数
            pretrained_path = val['pretrained'] if ('pretrained' in val) else None
            self.networks[key] = self.init_network(def_file, net_opt, pretrained_path, key)#这里在每个Key更新了networks
                                                                                           #如下所知，每个key里有一个模块化的network

    def init_network(self, net_def_file, net_opt, pretrained_path, key):#被上面调用了
        ...
        network = imp.load_source("",net_def_file).create_model(net_opt) #将network模块化
        if pretrained_path != None:
            self.load_pretrained(network, pretrained_path)

        return network

    def load_pretrained(self, network, pretrained_path):#加载预先训练好的network（参数）（测试用），将预先模块化的模块再转换为字典的操作
        self.logger.info('==> Load pretrained parameters from file %s:' % (pretrained_path))

        assert(os.path.isfile(pretrained_path)) #这个路径是不是一个文件，不是就报错
        pretrained_model = torch.load(pretrained_path) #把该路径的东西给到model
        if pretrained_model['network'].keys() == network.state_dict().keys(): #network所对应的字典（其实是已经模块化的模块）的key（元素）们是否和state_dict字典中的Key相同
                                                                              #state_dict()是pytorch里的方法，用来将模块转换成字典的
            network.load_state_dict(pretrained_model['network']) #将network里的东西重新转换成字典
        else:
    ...
    def init_all_optimizers(self):#优化器初始化
        self.optimizers = {}#为了使用torch.optim，需先构造一个优化器对象Optimizer，
                            #用来保存当前的状态，并能够根据计算得到的梯度来更新参数。 
                            #要构建一个优化器optimizer，
        ...
        for key, oparams in self.optim_params.items():
            ...
            if oparams != None:
                self.optimizers[key] = self.init_optimizer(#这里更新了optimizers
                        self.networks[key], oparams, key)

    def init_optimizer(self, net, optim_opts, key):#被上面调用
        ...
        if optim_type == 'adam': # Adam利用梯度的一阶矩估计和二阶矩估计动态调整每个参数的学习率。
            optimizer = torch.optim.Adam(parameters, lr=learning_rate,
                        betas=optim_opts['beta'])#Adam需要的参数
        elif optim_type == 'sgd':
    ...
    def init_all_criterions(self):#pytorch中的一个参数？，和Loss有关
    ...
    def init_criterion(self, ctype, copt):#被上面调用
        return getattr(nn, ctype)(copt)

    def load_to_gpu(self):#加载到gpu
    ...
    def solve(self, data_loader_train, data_loader_test):
        ...
        for self.curr_epoch in xrange(start_epoch, self.max_num_epochs):
            ...
            self.adjust_learning_rates(self.curr_epoch)#调整当前learning rate
            train_stats = self.run_train_epoch(data_loader_train, self.curr_epoch) #train_states训练状态，run一个epoch
            self.logger.info('==> Training stats: %s' % (train_stats))

            self.save_checkpoint(self.curr_epoch+1) # create a checkpoint in the current epoch #保存当前checkpoint
            if start_epoch != self.curr_epoch: # delete the checkpoint of the previous epoch
                self.delete_checkpoint(self.curr_epoch) #删除了旧的checkpoint
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        for key, network in self.networks.items(): #迭代networks中的每个项目
                                                   #每个key里都有一个network块
            if self.optimizers[key] == None: network.eval() #如果项目为空，则eval
                                                            #在init_optimizers中更新过，所以应该是有值的
            else: network.train() #如果项目不为空，则train（train,eval都是内置函数）
                                  #现在network是个模块，所以有train，这个train具体做了什么在别的文件里

        disp_step   = self.opt['disp_step'] if ('disp_step' in self.opt) else 50 #步长，这是啥
    ...
    def evaluate(self, dloader):
        ...
        for idx, batch in enumerate(tqdm(dloader())):
            self.biter = idx
            eval_stats_this = self.evaluation_step(batch) #空类，有继承
    ...
    def adjust_learning_rates(self, epoch):#过滤掉不可能实现的网络
    ...
    def keep_record_of_best_model(self, eval_stats, current_epoch): 
        if self.keep_best_model_metric_name is not None:#最佳距离
